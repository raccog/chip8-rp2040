...
typedef struct {
    ...
    byte delay;
    byte sound;
    volatile bool key_buffer[KEY_COUNT];
    bool keys[KEY_COUNT];
    bool should_draw;
...
rom_count_t uart_write_roms_to_flash() {
    ...
    return *(rom_count_t *)flash_target_contents;
}
...
void processor_load_font(processor_t *processor) {
    const byte FONT[] = {
...
void processor_clear_displays(processor_t *processor) {
...
word processor_next_instruction(processor_t *processor) {
    word instruction = ((word)processor->memory[processor->PC] << 8) | processor->memory[processor->PC + 1];
    processor->PC += 2;
    return instruction;
}

bool processor_update_pixel(processor_t *processor, word instruction, byte x, byte y, byte pixel) {
...
bool processor_get_key_pressed(processor_t *processor, uint8_t *key) {
...
void processor_update_keys(processor_t *processor) {
...
void processor_run_instruction(processor_t *processor, word instruction) {
    switch (get_opcode(instruction)) {
        case CLEAR_SCREEN:
            processor_clear_displays(processor);
        ...
        case DRAW:
            ...
            for (byte y = 0; y < get_N(instruction); ++y) {
                for (byte x = 0; x < 8; ++x) {
                    ...
                    if (processor_update_pixel(processor, instruction, x, y, pixel)) {
                        VF = 1;
                    }
                }
            }
        ...
        case WAIT_KEY_PRESS:
            {
                uint8_t key;
                while (!processor_get_key_pressed(processor, &key)) {
...
void processor_decrement_timers(processor_t *processor) {
...
int processor_run_cycle(processor_t *processor) {
...
int main() {
    ...
    printf("processor_t initialized\n");

    if (gpio_get(20)) {
        rom_count_t rom_count = uart_write_roms_to_flash();
    }
    rom_meta_t rom_meta;
    ...
    while (true) {
        if (processor_run_cycle(&processor) == 1) exit(1);
